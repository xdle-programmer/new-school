...
function loadFrontScripts()
{
    ...
    $js = '';
    $files = scandir('wp-content/themes/new-school/markup');
    ...
    wp_enqueue_script('script', get_template_directory_uri() . '/markup/' . $js, null, null, true);
}
...
function loadFrontStyles()
{
    ...
    wp_enqueue_style('style', get_template_directory_uri() . '/markup/' . $css);
}
...
function ajax_parent_auth()
{
    ...
    $post_url = 'https://smsc.ru/sys/send.php?login=radomlya&psw=Zcfy2vYXZdCK6MS&phones='.$number.'&mes=Ваш код: ' . $password;
    $request = new WP_Http();
